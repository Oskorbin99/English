...
int main()  {
   ...
   read_file("List of words.txt", en_words, ru_words, part_of_the_word, probability_words, count_words, subject_word, definition_word, status_word, transcription_word, ru_save_words);
   write_file("List of words.txt", en_words, ru_save_words, part_of_the_word, probability_words, count_words, subject_word, definition_word, transcription_word);
   ...
   while (while_ad == true){
       ...
       cout <<"Непроверенные слова - " <<count_new_words (probability_words,3) << endl;
       cout << "Проверенные слова - " << count_new_words(probability_words, 0) << endl;
       cout << "Слов с некоторым шансом на проверку - " << count_new_words(probability_words, -1) << endl;
        while (while_ad == true){
            
            random_word = rand() % count_words;
            if (rand() % 100 <= 2 && probability_words[random_word] == -1) probability_words[random_word] = 1;
